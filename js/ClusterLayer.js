define([
...
], function (declare, arrayUtils, lang, Color, connect,
  ...
  return declare([GraphicsLayer], {
    constructor: function (options) {
      ...
      this._singleSym = options.singleSymbol || new SimpleMarkerSymbol("circle", 6, null, new Color(options.singleColor || "#888"));
    ...
    _setMap: function (map, surface) {
      ...
      map.infoWindow.on("hide", lang.hitch(this, function (evt) {
    ...
    setData: function (data) {
      ...
      if(data && Array.isArray(data)) {
        ...
        this._clusterData = arrayUtils.filter(data, function (dataItem) {
          return (dataItem.geometry != null);
        });
      } else {
        this._clusterData = [];
      }
    ...
    clearSingles: function (singles) {
      ...
      arrayUtils.forEach(s, lang.hitch(this, this.remove));
    ...
    _clusterTest: function (p, cluster) {
      var distance = (
          Math.sqrt(Math.pow((cluster.x - p.x), 2) + Math.pow((cluster.y - p.y), 2)) / this._clusterResolution
      );
    ...
    _clusterAddPoint: function (p, cluster) {
      ...
      var x = (p.x + (cluster.x * count)) / (count + 1);
      var y = (p.y + (cluster.y * count)) / (count + 1);
      cluster.x = x;
      cluster.y = y;
    ...
    _showAllClusters: function () {
      arrayUtils.forEach(this._clusters, lang.hitch(this, this._showCluster));
      /*
    ...
    _addSingles: function (singles) {
      ...
      arrayUtils.forEach(singles, lang.hitch(this, function (p) {
        var g = new Graphic(new Point(p.x, p.y, this._sr), this._singleSym, p.attributes, this._singleTemplate);
    ...
    _updateClusterGeometry: function (c) {
      ...
      var cg = arrayUtils.filter(this.graphics, lang.hitch(this, function (g) {
    ...
    _updateLabel: function (c) {
      ...
      var label = arrayUtils.filter(this.graphics, lang.hitch(this, function (g) {
        return g.symbol && (g.symbol.declaredClass == "esri.symbol.TextSymbol") && (g.attributes.clusterId == c.attributes.clusterId);
      }));
      if(label.length == 1) {
        // console.log("update label...found: ", label);
        this.remove(label[0]);
        var newLabel = new TextSymbol(c.attributes.clusterCount).setColor(new Color(this._clusterLabelColor)).setOffset(0, this._clusterLabelOffset);
    ...
    _clusterMeta: function () {
      ...
      var count = 0;
      arrayUtils.forEach(this._clusters, lang.hitch(this, function (c) {
